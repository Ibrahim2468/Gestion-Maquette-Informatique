	...
	<div>
		<form method="post" action="<c:url value="/creationCommande"/>" enctype="multipart/form-data">
			<fieldset>
				...
				<%-- Si et seulement si la Map des clients en session n'est
					...
					<div id="ancienClient">
						<select name="listeClients" id="listeClients">
							...
							<c:forEach items="${ sessionScope.clients }" var="mapClients">
								<%-- L'expression EL ${mapClients.value} permet de
...
nom et prenom comme on le ferait avec n'importe quel bean. --%>
								<option value="${ mapClients.key }">${
									mapClients.value.prenom } ${ mapClients.value.nom }</option>
							</c:forEach>
						</select>
					</div>
				</c:if>
			...
			<fieldset>
				...
				<input type="text" id="v" name="dateCommande"
					value="<joda:format value="${ commande.date }" pattern="dd/MM/yyyy HH:mm:ss"/>"
					size="30" maxlength="30" disabled /> <span class="erreur">${form.erreurs['dateCommande']}</span>
...
- http://www.siteduzero.com/tutoriel-3-309961-dynamisez-vossites-
web-avec-javascript.html
- http://www.siteduzero.com/tutoriel-3-659477-un-site-webdynamique-
avec-jquery.html
