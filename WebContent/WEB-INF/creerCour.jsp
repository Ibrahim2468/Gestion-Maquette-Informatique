...
<body>
	...
	<c:import url="menu_classe.jsp" />
	
	

  <div class="row djina">
  <form class="col s12" method="post" action="CreationCour?val=${val }">
    <div class="row">
      <h2 class="teal-text header center-text">Ajout d'un Cour</h2>
	...
	<div class="input-field col s12">
	  <select name="enseignant">
	    ...
	    <c:forEach items="${enseignants}" var="news">
	    	<option value="${news.id}"> ${news.nom_enseignant }</option>
	    </c:forEach>
	  </select>
...
<!-- 	  <select name="classe"> -->
<!-- 	    <option value="" disabled selected></option> -->
<%-- 	    <c:forEach items="${classes}" var="news"> --%>
<%-- 	    	<option value="${news.id_classe}"> ${news.nom_classe }</option> --%>
<%-- 	    </c:forEach> --%>
<!-- 	  </select> -->
...
<!-- 	</div> -->
	<div class="input-field col s12">
	  <select name="matiere">
	    <option value="" disabled selected></option>
	    <c:forEach items="${matieres}" var="news">
	    	<option value="${news.id_matiere}"> ${news.intitule_matiere }</option>
	    </c:forEach>
	  </select>
	...
	<div class="input-field col s12">
	  <select name="salle">
	    <option value="" disabled selected></option>
	    <c:forEach items="${salles}" var="news">
	    	<option value="${news.id_salle}"> ${news.nom_salle }</option>
	    </c:forEach>
	  </select>
	  <label for="salle">Salle</label>
	</div>
	<div class="input-field col s12">
        <input id="datee" type="datetime" class="datepicker" name="datee" />
        <label for="datee">Date du cour</label>
    </div>
    <div class="input-field col s12">
        <input id="duree" type="number" class="validate" name="duree" />
        <label for="duree">Duree du cour</label>
    </div>
	<div class="input-field col s12">
		<textarea id="detail" class="materialize-textarea" name="detail" ></textarea>
		<label for="detail">Details du cour</label>
	</div>
...
<!-- 	    <a href="openclass.com"class="left waves-effect waves-light btn">button</a> -->
		...
		<a class="teal btn right" href="openclass.com"><i class="material-icons left">delete</i>ANNULER</a>
