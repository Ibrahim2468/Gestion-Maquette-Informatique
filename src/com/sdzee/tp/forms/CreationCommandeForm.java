...
public final class CreationCommandeForm {
	...
	public Commande creerCommande(HttpServletRequest request, String chemin) {
		...
		if (ANCIEN_CLIENT.equals(choixNouveauClient)) {
			...
			try {
				...
				setErreur(CHAMP_CHOIX_CLIENT,
						"Client inconnu, merci d'utiliser le formulaire prévu à cet effet.");
				id = 0L;
			}
			/*
			...
			client = ((Map<Long, Client>) session.getAttribute(SESSION_CLIENTS))
					.get(id);
		} else {
			/*
	...
	private double validationMontant(String montant)
	...
	private void validationModePaiement(String modePaiement)
		...
		if (modePaiement != null) {
			if (modePaiement.length() < 2) {
				throw new FormValidationException(
						"Le mode de paiement doit contenir au moins 2 caractères.");
			}
		} else {
			throw new FormValidationException(
	...
	private void validationStatutPaiement(String statutPaiement)
		...
		if (statutPaiement != null && statutPaiement.length() < 2) {
			throw new FormValidationException(
					"Le statut de paiement doit contenir au moins 2 caractères.");
		}
	}
	...
	private void validationModeLivraison(String modeLivraison)
		...
		if (modeLivraison != null) {
			if (modeLivraison.length() < 2) {
				throw new FormValidationException(
						"Le mode de livraison doit contenir au moins 2 caractères.");
			}
		} else {
			throw new FormValidationException(
					"Merci d'entrer un mode de livraison.");
		}
	}
	...
	private void validationStatutLivraison(String statutLivraison)
		...
		if (statutLivraison != null && statutLivraison.length() < 2) {
			throw new FormValidationException(
					"Le statut de livraison doit contenir au moins 2 caractères.");
		}
	}
